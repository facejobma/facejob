...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await fetch(
        `${(typeof window !== 'undefined' ? '' : process.env.NEXT_PUBLIC_BACKEND_URL)}/api/v1/admin/delete_cv/${data.id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const onVerify = async (isVerified: string) => {
    try {
      const response = await fetch(
        `${(typeof window !== 'undefined' ? '' : process.env.NEXT_PUBLIC_BACKEND_URL)}/api/v1/admin/verify/${data.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${authToken}`,
