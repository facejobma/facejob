...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      const response = await fetch(
        `${(typeof window !== 'undefined' ? '' : process.env.NEXT_PUBLIC_BACKEND_URL)}/api/v1/candidate/updateId/${candidateId}`,
        {
          method: "PUT",
          headers: {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await fetch(
        `${(typeof window !== 'undefined' ? '' : process.env.NEXT_PUBLIC_BACKEND_URL)}/api/v1/candidate/delete/${candidateId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authToken}`
